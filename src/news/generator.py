...
from .dedup import load_seen_articles, filter_unseen, mark_as_seen, save_seen_articles
...
class NewsGenerator:
    ...
    def generate_news_digest_from_sources(
        ...
        Stage 1: Analyze and select high-quality news items
        ...
        try:
            ...
            # --- Deduplication: filter out previously seen articles ---
            seen = load_seen_articles()
            original_count = len(news_data['international']) + len(news_data['domestic'])
            news_data['international'] = filter_unseen(news_data['international'], seen)
            news_data['domestic'] = filter_unseen(news_data['domestic'], seen)
            filtered_count = len(news_data['international']) + len(news_data['domestic'])
            deduped = original_count - filtered_count
            if deduped > 0:
                logger.info(f"Deduplication: removed {deduped} previously seen articles ({original_count} → {filtered_count})")

            if not news_data['international'] and not news_data['domestic']:
                error_msg = "All fetched news items were previously seen. No new content available."
                logger.error(error_msg)
                raise Exception(error_msg)
            ...
            # ============================================================
            # STAGE 1: Selection - Analyze and select best items
            # ============================================================
            ...
            if not json_match:
                ...
                selected_ids = list(news_items.keys())[:3]
            else:
                try:
                    selected_ids = json.loads(json_match.group(0))
                    # Validate IDs — keep only IDs that exist in our fetched news
                    selected_ids = [id for id in selected_ids if id in news_items]

                    if len(selected_ids) == 0:
                        logger.warning("No valid IDs after validation, using fallback")
                        selected_ids = list(news_items.keys())[:3]
                    else:
                        # Cap at 20 as a safety valve, but trust the LLM's count
            ...
            footer = "\n\n---\n\n*Generated by [Personal News Digest](https://github.com/bengraf9/ai-news-bot)*"
            response_text += footer

            # --- Deduplication: mark selected articles as seen ---
            selected_articles = [news_items[id] for id in selected_ids if id in news_items]
            seen = mark_as_seen(selected_articles, seen)
            save_seen_articles(seen)
            logger.info(f"Deduplication: marked {len(selected_articles)} articles as seen")
